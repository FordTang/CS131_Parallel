...
void* search_function(void *arg_struct)
{
    ...
    while (1)
    {
        pthread_mutex_lock(&argument->data->mutex);
        slice_number = argument->data->sv - 1;
        ++argument->data->sv;
        pthread_mutex_unlock(&argument->data->mutex);

        if (slice_number >= argument->data->number_of_slices)
            break;
        ...
        if (slice_number == (argument->data->number_of_slices - 1) && ((slice_number + 1) * argument->data->slice_size) != argument->data->number_of_lines)
            end = argument->data->number_of_lines;
        else
            end = (slice_number + 1) * argument->data->slice_size;

        printf("Thread Number: %d\nStart: %d\nEnd: %d\n", argument->thread_number, slice_number * argument->data->slice_size, end);

		unsigned int i;

        for (i = slice_number * argument->data->slice_size; i < end; ++i)
